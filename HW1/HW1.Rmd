...
```
  ...
  + Suburb with index 399 has the lowes median value of owner occupied homes. Values of other predictors are shown above.
...
```
  ...
  + 8 suburbs have more than 8 rooms per dwelling.
...
```
  ...
  + For the 8 suburbs that have on an avg more than 8 rooms per dwelling:
...
```{r}
      ...
      crime_train[train_sample$crim>median(train_sample$crim)]=1    
      train_sample$crim=crime_train
      
      #setting up response variable to be binary for test data
      crime_test=rep(0,253)
      crime_test[test_sample$crim>median(test_sample$crim)]=1          
...
```      
  ...
  + Important predictors: Most important predictors wiht low p value (<0.05) nitrogen oxide levels(nox), age of bui, distance to the employment center(dis) and accessibility to highways(rad)
